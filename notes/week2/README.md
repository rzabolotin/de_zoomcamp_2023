...
Т.е. ты делалешь свой код, оборачиваешь в деплойменты - а потом можешь подключить свой воркер к серверу и он будет выполнять задачи, которые ты ему написал. Также может слать уведомления.
