...
Осуществляется это за счет разбиения больших задач на маленькие, которые могут быть выполнены параллельно, и потом аггрегированы.  

Чем-то пооже на то, как устроен внутри BigQuery, но в отличие от BigQuery, Spark позволяет работать с данными, которые не хранятся в BigQuery, с любыми данными, на вашем железе.
